...
// A publicly-fillable field maps to one of these; assignee/section are handled
// server-side via form defaults and never exposed here.
const PRIORITY_OPTS = [
  { v: "", l: "—" },
  { v: "low", l: "Low" },
  { v: "medium", l: "Medium" },
...
export default function PublicFormPage() {
  ...
  const [state, setState] = useState("loading"); // loading | ready | error | submitting | done
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const init = {};
      (r.form.fields || []).forEach((f) => { init[f.id] = ""; });
      setForm(r.form);
      setValues(init);
      setState("ready");
    })();
  ...
  const submit = async () => {
    setError("");
    // client-side required check for nicer UX
    for (const f of (form.fields || [])) {
      if (f.required && !String(values[f.id] ?? "").trim()) { setError(`"${f.label}" is required.`); return; }
    }
    setState("submitting");
    const r = await callForm({ action: "submit", token, values });
    if (r.error) { setError(r.error); setState("ready"); return; }
    setState("done");
  };

  const page = {
    minHeight: "100vh", background: "#f4f5f7", color: "#1f2430",
  ...
  const card = {
    ...
    boxShadow: "0 10px 40px rgba(0,0,0,0.08)", border: "1px solid #e6e8ec",
    overflow: "hidden",
  };
  const label = { fontSize: 13, fontWeight: 600, color: "#3a4150", display: "block", marginBottom: 6 };
  const inp = {
    width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid #d6dae1",
    background: "#fff", color: "#1f2430", fontSize: 14, outline: "none", boxSizing: "border-box",
    fontFamily: "inherit",
  };

  const brand = (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 22, fontSize: 12, color: "#9aa1ac" }}>
      <span>🚀</span><span>Powered by Helm</span>
    </div>
  );

  if (state === "loading") {
    return <div style={page}><div style={{ ...card, padding: 40, textAlign: "center", color: "#9aa1ac" }}>Loading form…</div>{brand}</div>;
  }
  if (state === "error") {
    return (
      <div style={page}>
    ...
    return (
      <div style={page}>
        <div style={{ ...card, padding: 44, textAlign: "center" }}>
          ...
          <button onClick={() => { const init = {}; (form.fields || []).forEach((f) => { init[f.id] = ""; }); setValues(init); setState("ready"); }}
        ...
        <div style={{ padding: "28px 30px 30px" }}>
          ...
          {(form.fields || []).map((f) => (
            <div key={f.id} style={{ marginBottom: 18 }}>
              <label style={label}>{f.label}{f.required && <span style={{ color: "#ef4444" }}> *</span>}</label>
          ...
          <button onClick={submit} disabled={busy}
            style={{ width: "100%", padding: "12px 0", borderRadius: 10, border: "none", background: busy ? "#a5a8f0" : "#6366f1", color: "#fff", fontSize: 15, fontWeight: 700, cursor: busy ? "default" : "pointer", marginTop: 4 }}>
