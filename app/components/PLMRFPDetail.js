...
export default function PLMRFPDetail({ rfp: rfpInitial, program, onBack }) {
  ...
  const [selectedAgent, setSelectedAgent] = useState("rfp_writer");
  ...
  const aiChatSend = async () => {
    ...
    if (r?.reply) setAiChat([...newHist, { role: "assistant", content: r.reply, agent_name: r.agent_name, agent_emoji: r.agent_emoji }]);
  };
      ...
      {aiPanelOpen && (
        <div style={{ width: 380, flexShrink: 0, position: "sticky", top: 20, background: T.surface2, border: "1px solid " + T.border, borderRadius: 10, padding: 14, maxHeight: "calc(100vh - 80px)", overflow: "auto" }}>
          ...
          <div style={{ borderTop: "1px solid " + T.border, paddingTop: 10 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: T.text3, textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 6 }}>Chat with an expert agent</div>
              ...
              {aiChat.length === 0 && (
                <div style={{ padding: 10, fontSize: 11, color: T.text3, fontStyle: "italic", lineHeight: 1.5, background: T.surface, borderRadius: 6 }}>
                  {selectedAgent === "sourcing_expert"
                    ? "I find specialized CMs (not big CPG conglomerates) for any category and region. Try: \"Find me eco-friendly laundry detergent CMs in the US Midwest.\""
                    : "I write RFPs that get the best terms even when volume is uncertain. Try: \"Draft the pricing-tier structure for this RFP\" or \"What clarifying questions should I add?\""}
                </div>
              )}
              {aiChat.map((m, i) => (
            ...
            <div style={{ display: "flex", gap: 4 }}>
              <input value={aiInput} onChange={e => setAiInput(e.target.value)} onKeyDown={e => e.key === "Enter" && aiChatSend()} placeholder={selectedAgent === "sourcing_expert" ? "e.g. Find me CMs for citrus surfactants in the EU" : "e.g. Draft the pricing-tier table"} style={{ flex: 1, background: T.surface, border: "1px solid " + T.border, color: T.text, fontSize: 12, padding: "7px 10px", borderRadius: 5, outline: "none" }} />
