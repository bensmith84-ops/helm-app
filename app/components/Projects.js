...
export default function ProjectsView({ pendingTaskId, clearPendingTask, pendingProjectId, clearPendingProject }) {
  ...
  const sortedTasks = (list) => {
    if (sortCol === "sort_order") {
      return [...list].sort((a, b) => {
        const av = a.sort_order ?? 1e9, bv = b.sort_order ?? 1e9;
        if (av !== bv) return av - bv;
        return String(a.created_at || "").localeCompare(String(b.created_at || ""));
      });
    }
    if (sortCol === "sort_order") return list; return [...list].sort((a, b) => { let va = a[sortCol] || "", vb = b[sortCol] || ""; if (sortCol === "due_date") { va = va ? new Date(va).getTime() : 9e15; vb = vb ? new Date(vb).getTime() : 9e15; } const c = va < vb ? -1 : va > vb ? 1 : 0; return sortDir === "asc" ? c : -c; }); };
  ...
  const listViewEl = (() => { const TaskRow = _taskRowRef.current; const toggleSort = (col) => { setSortCol(col); setSortDir(p => sortCol === col && p === "asc" ? "desc" : "asc"); }; const arrow = (col) => sortCol === col ? (sortDir === "asc" ? " ↑" : " ↓") : ""; return (
    <div style={{ flex: 1, overflow: "auto", padding: "0 0 80px" }}>
      <style>{`
        .task-row .datecell-empty .datecell-inputs{visibility:hidden}
        .task-row:hover .datecell-empty .datecell-inputs{visibility:visible}
        .task-row .datecell-empty .datecell-ghost{position:absolute;left:0;top:50%;transform:translateY(-50%);font-size:11.5px;pointer-events:none;white-space:nowrap}
        .task-row:hover .datecell-empty .datecell-ghost{display:none}
      `}</style>
      <div style={{ display: "grid", gridTemplateColumns: listGrid, padding: isMobile ? "0 8px" : "0 12px", borderBottom: `1px solid ${T.border}`, position: "sticky", top: 0, zIndex: 5, background: T.bg }}>
        <div style={{ ...S.colHdr, position: "sticky", left: 0, zIndex: 6, background: T.bg }} onClick={() => toggleSort("title")}>Task name{arrow("title")}<ResizeHandle onDown={(e) => projResize(0, e)} /></div>
      ...
      {projSections.map((sec, si) => { const st = filteredTasks.filter(t => t.section_id === sec.id); const roots = sortedTasks(rootTasks(st)); const isColl = collapsed[sec.id]; const sd = st.filter(t => t.status === "done").length; const color = secColor(si);
        ...
        <div key={sec.id} style={{ background: dragOverSection === sec.id ? T.accentDim + "22" : "transparent", outline: dragOverSection === sec.id ? `2px dashed ${T.accent}66` : "none", outlineOffset: -2, borderRadius: 8, transition: "background 0.1s" }}
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "10px 12px", cursor: "pointer", userSelect: "none", position: "sticky", left: 0, width: "max-content", maxWidth: "100%", zIndex: 1 }}
          ...
          {!isColl && <>{roots.map(task => <TaskRow key={task.id} task={task} depth={0} />)}{addingTo === sec.id ? <div style={{ ...S.row(false, false), background: T.surface2 }}><div style={{ display: "flex", alignItems: "center", gap: 8, paddingLeft: 20, position: "sticky", left: 0, zIndex: 2, background: "inherit" }}><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={T.accent} strokeWidth="2"><path d="M12 5v14M5 12h14"/></svg><input value={newTitle} onChange={e => setNewTitle(e.target.value)} onKeyDown={e => { if (e.key === "Enter") createTask(sec.id); if (e.key === "Escape") { setAddingTo(null); setNewTitle(""); } }} onBlur={() => { if (newTitle.trim()) createTask(sec.id); else { setAddingTo(null); setNewTitle(""); } }} autoFocus placeholder="Task name…" style={{ flex: 1, background: "none", border: "none", color: T.text, fontSize: 13, outline: "none" }} /></div><div /><div /><div /><div /></div> : <div onClick={() => { setAddingTo(sec.id); setNewTitle(""); }} style={{ ...S.addRow, opacity: 0.6, position: "sticky", left: 0, width: "max-content" }} onMouseEnter={e => e.currentTarget.style.opacity = 1} onMouseLeave={e => e.currentTarget.style.opacity = 0.6}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 5v14M5 12h14"/></svg>Add task…</div>}</>}
        </div>); })}
...
function DateCell({ task, onUpdate }) {
  ...
  const bothEmpty = !task.start_date && !task.due_date;
  return (
    <div className={bothEmpty ? "datecell-empty" : ""} style={{ position: "relative" }} onClick={(e) => e.stopPropagation()}>
      {bothEmpty && <span className="datecell-ghost" style={{ color: T.text3 }}>＋ Add dates</span>}
      <div className="datecell-inputs" style={{ display: "flex", alignItems: "center", gap: 2 }}>
      <input type="date"
