...
export default function ProjectsView({ pendingTaskId, clearPendingTask }) {
  ...
  const S = {
    ...
    row: (hov, sel) => ({ display: "grid", gridTemplateColumns: projGrid, alignItems: "center", padding: "0 12px", minHeight: 36, borderBottom: `1px solid ${T.border}`, background: sel ? T.accentDim : "transparent", cursor: "pointer", transition: "background 0.08s" }),
  };
        ...
        return (
          <div key={sec.id}
            ...
            <div style={{ flex: 1, padding: 8, display: "flex", flexDirection: "column", gap: 6, overflow: "auto" }}>
              ...
              {addingTo === sec.id
                ? <div style={{ padding: 8 }}><input value={newTitle} onChange={e => setNewTitle(e.target.value)} onKeyDown={e => { if (e.key === "Enter") createTask(sec.id); if (e.key === "Escape") { setAddingTo(null); setNewTitle(""); } }} onBlur={() => { if (newTitle.trim()) createTask(sec.id); else { setAddingTo(null); setNewTitle(""); } }} placeholder="Task name…" style={{ width: "100%", padding: "6px 8px", borderRadius: 6, border: `1px solid ${T.border}`, background: T.surface, color: T.text, fontSize: 12, outline: "none" }} /></div>
