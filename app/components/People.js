...
export default function PeopleView() {
  ...
  const permScrollRef = { current: null };
  const permScrollRef = React.useRef(0);
  const toggleModuleAccess = async (uid, mod) => { const om = getMembership(uid); if (!om) return; const perms = om.module_permissions || {}; const current = perms[mod] !== false; const updated = { ...perms, [mod]: !current }; const scrollEl = document.querySelector("[data-perm-scroll]"); permScrollRef.current = scrollEl?.scrollTop || 0; const { error } = await supabase.from("org_memberships").update({ module_permissions: updated }).eq("id", om.id); if (error) return showToast("Failed to update permissions"); setMemberships(p => p.map(m => m.id === om.id ? { ...m, module_permissions: updated } : m)); setTimeout(() => { const el = document.querySelector("[data-perm-scroll]"); if (el) el.scrollTop = permScrollRef.current; }, 0); setTimeout(() => { const el = document.querySelector("[data-perm-scroll]"); if (el) el.scrollTop = permScrollRef.current; }, 50); };
