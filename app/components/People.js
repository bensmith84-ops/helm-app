...
const MODULE_TREE = [
  ...
  { key: "erp", label: "ERP", icon: "◧", children: [
    ...
    { key: "erp.balance_sheet", label: "Balance Sheet" },
    ...
    { key: "erp.fin_budget_planner", label: "Budget Planner" },
    ...
    { key: "erp.demand_planning", label: "Demand Planning" },
    ...
    { key: "erp.3pl_billing", label: "3PL Billing" },
    { key: "erp.reports", label: "Reports" },
  ]},
...
export default function PeopleView() {
  ...
  const createTeam = async () => {
    const nm = teamForm.name.trim();
    if (!nm) return showToast("Team name required");
    const base = nm.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 50) || "team";
    const payload = (slug) => ({ org_id: profile.org_id, name: nm, slug, description: teamForm.description, color: teamForm.color, parent_team_id: teamForm.parent_team_id || null, created_by: user.id });
    let { data, error } = await supabase.from("teams").insert(payload(base)).select().single();
    if (error && (error.code === "23505" || /duplicate|unique/i.test(error.message || ""))) { ({ data, error } = await supabase.from("teams").insert(payload(base + "-" + Math.random().toString(36).slice(2, 6))).select().single()); }
  ...
  if (loading) return <div style={{ display: "flex", height: "100%", alignItems: "center", justifyContent: "center", color: T.text3, fontSize: 13 }}><div style={{ width: 28, height: 28, border: `3px solid ${T.border}`, borderTopColor: T.accent, borderRadius: "50%", animation: "spin 0.8s linear infinite", marginRight: 10 }} />Loading users…<style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>;
