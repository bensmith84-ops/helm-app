...
const ERP_NAV = [
  ...
  { id: "fin_budget_planner", label: "Budget Planner", icon: "🧭" },
  ...
  useEffect(() => {
    const load = async () => {
      const [
        ...
        supabase.from("erp_bom").select("*").eq("org_id", orgId).order("created_at"),
        supabase.from("erp_bom_items").select("*").eq("org_id", orgId).order("sort_order"),
        ...
        supabase.from("erp_inventory_lots").select("*").eq("org_id", orgId).order("created_at", { ascending: false }),
        supabase.from("erp_purchase_orders").select("*").eq("org_id", orgId).order("created_at", { ascending: false }),
        supabase.from("erp_po_items").select("*").eq("org_id", orgId),
        supabase.from("erp_orders").select("*").eq("org_id", orgId).order("order_date", { ascending: false }),
        supabase.from("erp_order_items").select("*").eq("org_id", orgId),
        supabase.from("erp_customers").select("*").eq("org_id", orgId).order("name"),
        supabase.from("erp_work_orders").select("*").eq("org_id", orgId).order("created_at", { ascending: false }),
        supabase.from("erp_entities").select("*").eq("org_id", orgId).order("code"),
        supabase.from("erp_currencies").select("*").eq("org_id", orgId).eq("is_active", true).order("code"),
        supabase.from("erp_exchange_rates").select("*").eq("org_id", orgId).order("effective_date", { ascending: false }),
        supabase.from("erp_supplier_items").select("*").eq("org_id", orgId).order("item_name"),
        supabase.from("erp_inventory_movements").select("*").eq("org_id", orgId).order("created_at", { ascending: false }).limit(200),
        supabase.from("erp_carriers").select("*").eq("org_id", orgId).order("name"),
        supabase.from("erp_carrier_services").select("*").eq("org_id", orgId).order("carrier_id, name"),
        supabase.from("erp_fulfillment_integrations").select("*").eq("org_id", orgId).order("name"),
        supabase.from("erp_rma").select("*").eq("org_id", orgId).order("created_at", { ascending: false }),
        supabase.from("erp_rma_items").select("*").eq("org_id", orgId),
        ...
        supabase.from("erp_journal_entries").select("*").eq("org_id", orgId).order("entry_date", { ascending: false }).limit(100),
        supabase.from("erp_bin_locations").select("*").eq("org_id", orgId).order("code"),
        supabase.from("erp_shipping_rules").select("*").eq("org_id", orgId).order("priority"),
        supabase.from("erp_landed_costs").select("*").eq("org_id", orgId).order("created_at"),
        supabase.from("erp_journal_lines").select("*").eq("org_id", orgId).order("sort_order"),
        supabase.from("erp_credit_memos").select("*").eq("org_id", orgId).order("created_at", { ascending: false }),
      ]);
      ...
      try {
        const [{ data: qa }, { data: qv }, { data: qb }, { data: qc }, { data: qi }, { data: qp }] = await Promise.all([
          supabase.from("qbo_accounts").select("*").eq("org_id", orgId).order("account_type").order("name"),
          supabase.from("qbo_vendors").select("*").eq("org_id", orgId).order("display_name"),
    ...
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minWidth: 0 }}>
        ...
        <div style={{ flex: 1, overflow: "auto", padding: isMobile ? "10px 10px 20px" : "20px 24px" }}>
          ...
          {["cfo_dash", "pl_explorer", "cash_flow", "vendor_intel", "ap_aging", "txn_search", "revenue", "fin_requests", "fin_budgets", "fin_budget_planner", "fin_rules", "fin_audit"].some(v => view === v) && (
            <Suspense fallback={<div style={{ padding: 40, textAlign: "center", color: T.text3, fontSize: 13 }}>Loading Finance…</div>}>
              <FinanceEmbed initialView={{ cfo_dash: "cfo", pl_explorer: "pl_explorer", cash_flow: "cash_flow", vendor_intel: "vendors", ap_aging: "ap_aging", txn_search: "txn_search", revenue: "revenue", fin_requests: "requests", fin_budgets: "budgets", fin_budget_planner: "budget_planner", fin_rules: "rules", fin_audit: "audit" }[view] || "cfo"} embedded={true} modulePerms={modulePerms} />
            </Suspense>
          )}
        </div>
      </div>
