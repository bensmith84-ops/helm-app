...
const ACCESS_CODE = "EB-PARCEL-2026";
...
// content field schema: how each JSON key maps to an editor
const FIELDS = [
...
export default function ThreePLParcelRFP() {
  ...
  const loadContent = useCallback(async () => {
    ...
    if (!error && data?.content) setDraft(toDraft(data.content));
    setContentLoading(false);
  }, []);

  useEffect(() => { loadSubs(); loadContent(); }, [loadSubs, loadContent]);

  const save = async () => {
    setSaving(true); setErr(null);
    try {
      const content = fromDraft(draft);
  ...
  const exportCSV = () => {
    ...
    const csv = [cols.join(","), ...subs.map(s => cols.map(c => esc(s[c])).join(","))].join("\n");
    ...
    <div style={{ maxWidth: 1000 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
        <div style={{ display: "flex", gap: 2, background: T.surface2, borderRadius: 8, padding: 3 }}>
          {[["submissions", `Submissions${subs.length ? ` (${subs.length})` : ""}`], ["content", "Portal Content"]].map(([k, l]) => (
        ...
        <div style={{ flex: 1 }} />
        <span style={{ fontSize: 11.5, color: T.text3 }}>Access code: <b style={{ color: T.text2 }}>{ACCESS_CODE}</b></span>
        ...
        <div>
          <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
            {Object.entries(TYPE_META).map(([k, m]) => (
              <span key={k} style={{ fontSize: 11.5, padding: "3px 10px", borderRadius: 99, background: m.bg, color: m.fg, fontWeight: 700 }}>{counts[k] || 0} {m.label.toLowerCase()}{(counts[k] || 0) === 1 ? "" : "s"}</span>
            ))}
            <div style={{ flex: 1 }} />
          ...
          {!subsLoading && !subs.length && (
            <div style={{ ...card, padding: 36, textAlign: "center", color: T.text3, fontSize: 13 }}>
              No submissions yet. Carriers submit through the portal — intents, questions, and proposals all land here.
            </div>
          )}
            ...
            return (
              <div key={s.id} style={{ ...card, marginBottom: 8, overflow: "hidden" }}>
                <div onClick={() => setExpanded(open ? null : s.id)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 14px", cursor: "pointer" }}>
                  <span style={{ fontSize: 11, padding: "2px 9px", borderRadius: 99, background: m.bg, color: m.fg, fontWeight: 700, flexShrink: 0 }}>{m.label}</span>
      ...
      {tab === "content" && (
        <div>
          <div style={{ ...card, padding: "10px 14px", marginBottom: 14, fontSize: 12, color: T.text2, display: "flex", alignItems: "center", gap: 10 }}>
            <span>💡</span>
            <span>Edits save to the database and go <b>live on the portal immediately</b> — no redeploy. Visitors see changes on their next page load. HTML tags like &lt;b&gt; are allowed. The download files, weight strip, and submission form are fixed; ask Claude to regenerate those.</span>
          </div>
